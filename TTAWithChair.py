...
chair_npy = 'prob_dense0.npy'
chair = np.load(chair_npy)
chair_result = open('chair_result.txt', 'wb')
len2 = chair.shape[0]
print len2
for j in xrange(len2):
    res = int(np.where(chair[j] == np.max(chair[j]))[0][0])
    chair_result.write(str(res)+'\n')
chair_result.close()

def get_chair_result(filename):
    file = open(filename)
    res_line = []
    for i in xrange(896):
        line = file.readline()
...
def get_chair_id(filename):
    ...
    for i in xrange(896):
        ...
        line = line.strip()
        line = line.split(" ")
        res_line.append(int(line[1]))
    return res_line

chair_id = get_chair_id('chair_test.txt')
chair_res = get_chair_result('chair_result.txt')

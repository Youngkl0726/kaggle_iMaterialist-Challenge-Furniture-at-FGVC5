...
parser.add_argument('--port', default='24366', type=str)
...
class FinetunePretrainedModels(nn.Module):
    ...
    def forward(self, x):
        return self.net(x)
model_dict = {
    'se_resnext50_32x4d': partial(FinetunePretrainedModels, NB_CLASSES, senet.se_resnext50_32x4d)
}
...
def main():
    ...
    # create model
    print("=> creating model '{}'".format("se_resnext50_32x4d"))
    print("save_path is: {}".format(args.save_path))

    image_size = 256
    input_size = 224
    model = get_model('se_resnext50_32x4d', pretrained=True)
